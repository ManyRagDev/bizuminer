...
const catalogGuide = "O verde informa o desconto exibido no anúncio. A leitura de preço mostra quantos registros temos e só destaca um menor preço quando o histórico sustenta essa comparação.";
...
function SearchForm({ className, id, placeholder, query, disabled, onQuery, onSubmit }: { className: string; id: string; placeholder: string; query: string; disabled: boolean; onQuery: (value: string) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <form className={className} onSubmit={onSubmit} role="search">
    <span aria-hidden="true">⌕</span>
    <label className="sr-only" htmlFor={id}>Buscar produtos</label>
    <input id={id} value={query} onChange={(event) => onQuery(event.target.value)} placeholder={placeholder} />
    <button type="submit" disabled={disabled}>buscar</button>
  </form>;
}

function CategoryList({ className, categories, active, onChoose }: { className: string; categories: string[]; active: string | null; onChoose: (category: string | null) => void }) {
  return <div className={className}>
    <button className={active === null ? "active" : ""} onClick={() => onChoose(null)}>todas</button>
    {categories.map((item) => <button key={item} className={active === item ? "active" : ""} onClick={() => onChoose(item)}>{item}</button>)}
  </div>;
}
...
export default function Vitrine({ initialProducts, initialTotal, initialState, categories, dateLabel }: { initialProducts: VitrineProduct[]; initialTotal: number; initialState: CatalogState; categories: string[]; dateLabel: string }) {
  ...
  return <main className="vitrine-page">
    ...
    <header className="site-header">
      <a className="brand" href="#topo" aria-label="BizuMiner, início">
        <Image src="/brand/bizuminer-logo-horizontal-light.svg" alt="BizuMiner" width={140} height={32} priority className="brand-logo-desktop" />
        <Image src="/brand/bizuminer-icon-light.svg" alt="BizuMiner" width={32} height={32} priority className="brand-logo-mobile" />
      </a>
      <SearchForm className="search-box header-search" id="busca-mobile" placeholder="Buscar oferta" query={query} disabled={isLoading} onQuery={setQuery} onSubmit={submitSearch} />
    ...
    <section className="newsletter"><div><span className="newsletter-icon"><Image src="/brand/bizuminer-icon-square-blue.svg" alt="" width={44} height={44} className="rounded-xl" /></span></div><div><p className="eyebrow">Bizus antes de todo mundo</p><h2>As próximas ofertas chegam na sua caixa.</h2></div><form onSubmit={submitEmail}><label className="sr-only" htmlFor="email">Seu melhor e-mail</label><input id="email" type="email" required placeholder="seu melhor e-mail" /><button type="submit">quero receber <span>→</span></button></form></section>
    <footer><a className="brand footer-brand" href="#topo"><Image src="/brand/bizuminer-logo-horizontal-light.svg" alt="BizuMiner" width={130} height={30} className="brand-logo-img" /></a><p>A gente acompanha preços para trazer ofertas que valem a sua atenção.</p><div><a href="#criterios">transparência</a><a href="#vendedores">vendedores</a><a href="mailto:oi@bizuminer.com.br">contato</a></div><small>Alguns links são de afiliado. Podemos receber comissão sem custo adicional para você.</small></footer>
    ...
    {mobileView === "guide" && <MobilePanel view="guide" title="Como ler os preços" onClose={() => setMobileView("home")}><p className="mobile-panel-intro">{catalogGuide}</p><ul className="guide-legend"><li><span className="signal-badge verified">menor preço no período monitorado</span><p>O preço atual é o menor entre os registros que coletamos, e o histórico já é longo o bastante para sustentar a comparação.</p></li><li><span className="signal-badge monitoring">8 registros · 12 dias</span><p>Ainda estamos acompanhando. Mostramos quantos registros existem em vez de transformar pouca informação em certeza.</p></li><li><span className="signal-badge new">primeiro registro de preço</span><p>Vimos este produto uma vez só. Não há histórico para comparar.</p></li><li><span className="guide-discount">−38% no anúncio</span><p>Desconto informado pelo próprio anúncio no Mercado Livre — não é um cálculo nosso.</p></li></ul></MobilePanel>}
