...
export default function Vitrine({ initialProducts, initialTotal, initialState, categories, dateLabel, initialSavedIds = [], marketplaceCounts = {}, isUserAdmin = false }: { initialProducts: VitrineProduct[]; initialTotal: number; initialState: CatalogState; categories: string[]; dateLabel: string; initialSavedIds?: string[]; marketplaceCounts?: Record<string, number>; isUserAdmin?: boolean }) {
  ...
  function toggleFavorite(product: VitrineProduct) {
    ...
    showNotice(saved ? "Removido dos seus salvos." : "Produto salvo nos seus salvos.");
  ...
  return <main className="vitrine-page">
    ...
    <header className="site-header">
      ...
      <nav id="main-nav" className={navOpen ? "open" : ""} aria-label="Navegação principal"><a href="#achados" onClick={() => setNavOpen(false)}>Ofertas</a><a href="#criterios" onClick={() => setNavOpen(false)}>Como escolhemos</a><a href="#vendedores" onClick={() => setNavOpen(false)}>Para vendedores</a><a href="/minha-area" onClick={() => setNavOpen(false)}>Minha área</a>{isUserAdmin && <a href="/admin" onClick={() => setNavOpen(false)} style={{ color: "var(--blue-text, #1d4ed8)", fontWeight: 800 }}>⚡ Painel Admin</a>}</nav>
