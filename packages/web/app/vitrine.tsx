...
export default function Vitrine({ initialProducts, initialTotal, initialState, categories, dateLabel, initialSavedIds = [] }: { initialProducts: VitrineProduct[]; initialTotal: number; initialState: CatalogState; categories: string[]; dateLabel: string; initialSavedIds?: string[] }) {
  ...
  function toggleFavorite(product: VitrineProduct) {
    ...
    // Espelha no servidor (área do comprador); o localStorage segue valendo como cache local.
    void fetch("/api/minha/favoritos", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ productId: product.id, saved: !saved }),
    }).catch(() => {});
  }
  ...
  return <main className="vitrine-page">
    ...
    <header className="site-header">
      ...
      <nav id="main-nav" className={navOpen ? "open" : ""} aria-label="Navegação principal"><a href="#achados" onClick={() => setNavOpen(false)}>Ofertas</a><a href="#criterios" onClick={() => setNavOpen(false)}>Como escolhemos</a><a href="#vendedores" onClick={() => setNavOpen(false)}>Para vendedores</a><a href="/minha-area" onClick={() => setNavOpen(false)}>Minha área</a></nav>
